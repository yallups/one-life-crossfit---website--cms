...
function normalizeWodifyLocation(raw: any): NormalizedLocation {
  ...
  return {
    ...
    telephone: raw.phone ?? raw.phoneNumber ?? raw.telephone ?? raw.phone_number ?? null,
    address1: raw.address1 ?? raw.address ?? raw.street ?? raw.street_address_1 ?? null,
    city: raw.city ?? null,
    ...
    googleMapsUrl: raw.googlePlaceUrl ?? raw.googleMapsUrl ?? raw.mapsUrl ?? null,
  }
}
...
export async function getLocations(params: Record<string, string | number | boolean> = {}): Promise<LocationsResult> {
  // Try Wodify first
  try {
    const res = await getWodifyLocations(params)
    const raw = Array.isArray(res?.items) ? res.items : []
    const items = raw.map((r) => normalizeWodifyLocation(r)).filter(Boolean) as NormalizedLocation[]
...
export async function getPrimaryLocation(params: Record<string, string | number | boolean> = {}): Promise<PrimaryLocationResult> {
