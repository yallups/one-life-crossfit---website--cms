...
export default async function MemberDetailPage(props: {
  ...
  const res = await fetch(`${origin}/api/leaderboard/${challenge}/${year}/member/${encodeURIComponent(memberIdDecoded)}`, {
    cache: "no-store",
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Stat label="Habit Points" value={Number(detail.habitPoints).toFixed(2)} />
        <Stat label="Performance Points" value={Number(detail.performancePoints).toFixed(2)} />
      ...
      <section className="mb-8">
        ...
        <div className="overflow-auto rounded-md border border-border">
          <table className="w-full border-collapse text-sm">
            <thead
              className="sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/75">
            <tr className="text-[11px] uppercase text-muted-foreground">
              <th className="border-b border-border px-2 py-2 text-left">Date</th>
              {habits.map((h) => (
                <th key={h.key} className="border-b border-border px-2 py-2 text-center">{h.label}</th>
              ))}
              <th className="border-b border-border px-2 py-2 text-right">Daily</th>
            </tr>
            </thead>
            <tbody>
            {Array.isArray(detail.dailyWindows) && detail.dailyWindows.length > 0 ? (
              detail.dailyWindows.map((w: any) => (
                <Fragment key={w.date}>
                  {w.submissions.map((s: any, idx: number) => {
                    const counted = idx === w.countedIndex;
                    const rowTitle = !counted && s.notCountedReason ? s.notCountedReason : undefined;
                    return (
                      <tr key={w.date + "|" + s.timestamp}
                        ...
                        {habits.map((h) => {
                          ...
                          const award = counted ? w.perHabitAwards?.[h.key] : undefined;
                          const zeroed = counted && attempted && award && Number(award.awarded) === 0;
                          const reasons = zeroed ? (award.reasons || []) : [];
                          const title = counted ? (zeroed ? `${h.label} — not counted: ${reasons.join("; ")}` : h.label) : (rowTitle || h.label);
                          const baseClasses = [
                            "inline-flex size-4 items-center justify-center rounded-sm border",
                            attempted ? "bg-primary text-primary-foreground border-primary" : "border-border",
                          ];
                          const classes = zeroed ? baseClasses.concat(["line-through opacity-70", "bg-transparent text-muted-foreground border-dashed border-border"]) : baseClasses;
                          return (
                            <td key={h.key + idx} className="px-2 py-2 text-center">
                                <span
                        ...
                        <td className="px-2 py-2 text-right tabular-nums font-medium">
                          {counted ? Number(w.dailyPointsAwarded ?? 0).toFixed(2) : "—"}
              ...
              detail.dailyLogs.map((d: any) => (
                <tr key={d.date} className="border-b border-border/60">
                  <td className="px-2 py-2 text-left tabular-nums">{d.date}</td>
                  {habits.map((h) => {
                  ...
                  <td className="px-2 py-2 text-right tabular-nums font-medium">{Number(d.dailyPoints).toFixed(2)}</td>
                </tr>
              ))
            )}
            </tbody>
      ...
      <section>
        ...
        <p
          className="mt-2 text-xs text-muted-foreground">Updated: {new Date(detail.updatedAt).toLocaleString("en-US", { timeZone: cfg.timezone })}</p>
      </section>
    </main>
  );
}
