---
slug: /programs/private-training/semi-private
seo_title: 
seo_description: Semi-private training for 2–4 people with customized programming, shared accountability, and coach-led sessions that fit your schedule and goals.
og_title: 
og_description: 
primary_keyword: semi-private training Santa Maria
secondary_keywords: ["small group personal training Santa Maria", "partner training Santa Maria", "semi-private coaching"]
image_notes: Use small-group coaching or partner training imagery.
---

# Semi-Private Training

## Page Builder Blocks (ordered)
- hero: Hero
- layout: Intro
- featureCardsIcon: Value Props / Benefits
- layout: Program Details / Offering
- layout: Class Format / What To Expect
- imageLinkCards: Top Memberships
- cta: CTA / Next Step
- faqAccordion: FAQs

## Blocks (config-ready)
- hero
  - variant: centered
  - title: Semi-Private Training
  - richText: Personalized coaching in a small-group setting—perfect for partners or friends who want focused results.
  - buttons:
    - text: Book A Free Consultation
      url: { type: internal, internal: /free-consultation }
    - text: Learn About Memberships
      url: { type: internal, internal: #memberships-options }
- layout (Intro)
  - variant: centered
  - title: Personal coaching with shared motivation
  - richText: Semi-private training gives you individualized coaching while training alongside a partner or small group.
- featureCardsIcon (Benefits)
  - title: Benefits
  - cards:
    - title: Tailored coaching
      richText: Coaching customized for each person.
    - title: Shared accountability
      richText: Train with a partner or small group.
    - title: More attention than classes
      richText: Focused feedback each session.
    - title: Flexible scheduling
      richText: Sessions that fit your schedule.
- layout (Program Details)
  - variant: centered
  - title: Program details
  - richText: Small group sessions with customized programming, individual goal reviews, and progress tracking.
- layout (Class Format)
  - variant: centered
  - title: What to expect
  - richText: 2–4 athletes per coach, sessions designed around shared goals and individual needs.
- imageLinkCards (Top Memberships)
  - title: Top Memberships
  - cards:
    - title: Private Coaching
      description: Semi-private or 1:1 coaching tailored to your goals.
      url: { type: internal, internal: /memberships/private-coaching }
    - title: Hybrid
      description: Group classes plus personal coaching sessions (1/2/4/8 per 4 weeks).
      url: { type: internal, internal: /memberships/hybrid }
  - buttons:
    - text: See All Options
      url: { type: internal, internal: /memberships }
- cta
  - title: Want focused coaching with a partner?
  - richText: Book a free consultation and we’ll help you set up the right semi-private plan.
  - buttons:
    - text: Book A Free Consultation
      url: { type: internal, internal: /free-consultation }
    - text: Learn About Memberships
      url: { type: internal, internal: #memberships-options }
- faqAccordion
  - title: FAQs
  - faqs:
    - faq-semiprivate-size
    - faq-semiprivate-goals
    - faq-semiprivate-schedule
    - faq-semiprivate-nutrition

## Hero
- headline: Semi-Private Training
- subheadline: Personalized coaching in a small-group setting—perfect for partners or friends who want focused results.
- primary_cta: Book A Free Consultation
- primary_cta_link: https://onelifecrossfit.com/free-consultation
- secondary_cta: Learn About Memberships
- secondary_cta_link: #memberships-options
- hero_image_notes: Small-group or partner coaching image.

## Intro
- headline: Personal coaching with shared motivation
- body: Semi-private training gives you individualized coaching while training alongside a partner or small group. You’ll get focused attention and a supportive environment.

## Value Props / Benefits
- Coaching tailored to each person in the group
- More attention than a class, more energy than 1:1
- Great for partners, friends, or small teams
- Flexible scheduling and accountability

## Program Details / Offering
- Small group sessions with customized programming
- Individual goal reviews and progress tracking
- Movement coaching and scaling for each participant

## Class Format / What To Expect
- 2–4 athletes per coach (typical)
- Sessions designed around your shared goals and individual needs
- Clear progression and coaching feedback every session

## Top Memberships (anchor: memberships-options)
- description: Highlight the best-fit memberships for semi-private training.
- options:
  - name: Private Coaching
    summary: Semi-private or 1:1 coaching tailored to your goals.
    link: /memberships/private-coaching
  - name: Hybrid
    summary: Group classes plus personal coaching sessions (1/2/4/8 per 4 weeks).
    link: /memberships/hybrid
- see_all_options_link: /memberships

## CTA / Next Step
- headline: Want focused coaching with a partner?
- body: Book a free consultation and we’ll help you set up the right semi-private plan.
- primary_cta: Book A Free Consultation
- primary_cta_link: https://onelifecrossfit.com/free-consultation
- secondary_cta: Learn About Memberships
- secondary_cta_link: #memberships-options

## FAQs
- Q: How many people can be in a semi-private group?
  A: Typically 2–4 people per coach, depending on goals and scheduling.
- Q: Can partners have different goals?
  A: Yes. Sessions are tailored to each person’s needs even if goals differ.
- Q: Do we need the same schedule?
  A: You’ll book sessions together, so aligned schedules work best.
- Q: Can we add nutrition coaching?
  A: Yes. We can include nutrition/lifestyle coaching if desired.

## Internal Links
- /memberships/private-coaching
- /about

## Notes
- Wodify appointment service: Private Training (service ID 18760).
- Semi-private pack example: 1 session pack (template ID 244966).
